...
color_dict = {}
color_dict['RF-motif-epi']="#fa2111"
...
def parse_csv(x):
    ...
    return df
tmp_df = X_RF.copy()
tmp_df['tmp_index'] = tmp_df.index.tolist()
tmp_df.index = [x[:-1] for x in tmp_df.index]
print (tmp_df.head())
pos = glob.glob("pos*%s.average.csv.csv"%(5))[0]
neg = glob.glob("neg*%s.average.csv.csv"%(5))[0] 
pos = parse_csv(pos)
neg = parse_csv(neg)
tmp_X = pd.concat([pos,neg])
...
df_list = []
X_dict = {}
X_dict['RF-motif-epi']=X_RF
X_dict['DeepSEA']=X_DeepSEA
X_dict['CADD']=X_CADD
X_dict['LS-GKM']=X_gkm 
## RF top features
plot_top_features(dp(model),X_RF,Y,"motif-epi")
...
def parallel_unit(model,X_dict,Y):
	...
	return out_dict
out_list = Parallel(n_jobs=-1)(delayed(parallel_unit)(model,X_dict,Y) for i in range(50))
